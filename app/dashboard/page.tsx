  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-medium text-navy">Ecossistema da economia azul no Brasil</h1>
        <form className="flex gap-2" method="get">
          <select name="subsetor" defaultValue={searchParams.subsetor ?? ''} className="rounded border border-gray-300 px-3 py-2 text-sm">
            <option value="">Todos os subsetores</option>
            {SUBSETORES.map((s) => (
              <option key={s.valor} value={s.valor}>{s.label}</option>
            ))}
          </select>
          <select name="uf" defaultValue={searchParams.uf ?? ''} className="rounded border border-gray-300 px-3 py-2 text-sm">
            <option value="">Todos os estados</option>
            {ESTADOS_COSTEIROS.map((uf) => (
              <option key={uf} value={uf}>{uf}</option>
            ))}
          </select>
          <button type="submit" className="rounded border border-gray-300 px-4 py-2 text-sm hover:bg-gray-50">
            Filtrar
          </button>
        </form>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <h2 className="mb-3 text-lg font-medium text-navy">Ecossistema por subsetor</h2>
          <div className="space-y-2 rounded-xl border border-gray-200 bg-white p-4">
            {dados.empresasPorSubsetor.length === 0 && (
              <p className="text-sm text-gray-500">Nenhuma empresa cadastrada ainda para este filtro.</p>
            )}
            {dados.empresasPorSubsetor.map((item) => (
              <div key={item.subsetor} className="flex items-center gap-3">
                <span className="w-52 truncate text-sm text-gray-700">{labelSubsetor(item.subsetor)}</span>
                <div className="h-2 flex-1 rounded-full bg-gray-100">
                  <div
                    className="h-2 rounded-full bg-teal"
                    style={{ width: `${Math.round((item._count / maxEmpresas) * 100)}%` }}
                  />
                </div>
                <span className="w-8 text-right text-sm text-gray-600">{item._count}</span>
              </div>
            ))}
          </div>

          <h2 className="mb-3 mt-8 text-lg font-medium text-navy">Empresas em destaque</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {dados.empresasDestaque.map((empresa) => (
              ...
              >
                <p className="font-medium text-gray-900">{empresa.nomeFantasia || empresa.razaoSocial}</p>
                <p className="mt-1 text-sm text-gray-500">{labelSubsetor(empresa.subsetor)}</p>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-xs text-gray-400">{empresa.municipio}/{empresa.uf}</span>
                  <span className="rounded bg-lightblue px-2 py-0.5 text-xs text-teal">{labelEstagio(empresa.estagio)}</span>
                </div>
              </Link>
            ))}
            {dados.empresasDestaque.length === 0 && (
              <p className="text-sm text-gray-500">
                ...
                <Link href="/cadastro" className="text-teal underline">Cadastre a primeira</Link>.
              </p>
            )}
          </div>
        </div>
        ...
        <div>
          <h2 className="mb-3 text-lg font-medium text-navy">Oportunidades recentes</h2>
          <div className="space-y-3">
            {dados.oportunidadesProximas.map((op) => (
              <div key={op.id} className="rounded-xl border border-gray-200 bg-white p-3">
                <p className="text-xs font-medium text-teal">{op.tipo.replace('_', ' ')}</p>
                <p className="mt-1 text-sm text-gray-900">{op.titulo}</p>
                <p className="mt-1 text-xs text-amber-600">{diasRestantes(op.prazoInscricao)}</p>
              </div>
            ))}
            {dados.oportunidadesProximas.length === 0 && (
              <p className="text-sm text-gray-500">Nenhuma oportunidade cadastrada ainda.</p>
            )}
          </div>
        </div>
      </div>
...
function MetricCard({ label, valor, nota }: { label: string; valor: number; nota?: string }) {
  return (
